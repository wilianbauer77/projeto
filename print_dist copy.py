...
limit=10.0**(-300)
#Distribuição bimaxwelliana
def dist[0](vpar, vper, n,m,beta_par,beta_per,drift):
	bimax=numpy.exp(-n*(vpar-drift)**2/beta_par/m -n*vper**2/beta_per/m)* n**1.5 /(m**1.5 *numpy.pi**1.5 *beta_per*numpy.sqrt(beta_par))
	return bimax.ravel()

#Distribuição bikappa
def dist[1](vpar, vper, n, kappa,theta_par,theta_per,drift):
...
for ki in range(0,nkappa):
	for ispecies in range(0,Nspecies):
		for f in range(0,3):
			...
			file_name_bikappa='distribution_bikappa_'+str(K[ispecies][ki])+'_'+str(ispecies+1)+'.dat'
			...
			databimax=dist[f](vpara2, vperp2,dens[ispecies],mu[ispecies],beta_para[ispecies],beta_perp[ispecies],vdrift[ispecies])
